...
std::vector<causal_slam::telemetry::StreamTimingDiagnostic> BuildTimingDiagnostics(
    const causal_slam::telemetry::TimingSummary& imu_summary, const causal_slam::telemetry::TimingSummary& lidar_summary) {
  return {
      causal_slam::telemetry::MakeStreamTimingDiagnostic(causal_slam::telemetry::TemporalStreamId::kImu, imu_summary),
      causal_slam::telemetry::MakeStreamTimingDiagnostic(causal_slam::telemetry::TemporalStreamId::kLidar, lidar_summary),
  };
}

causal_slam::model::PointTimeDiagnostics BuildPointTimeDiagnostics(const causal_slam::pointcloud::PointCloud2FieldInspection& inspection) {
  ...
  if (diagnostic_field != nullptr) {
    ...
    diagnostics.field_datatype = causal_slam::pointcloud::PointCloud2DatatypeToString(diagnostic_field->datatype);
    diagnostics.field_role = causal_slam::pointcloud::ToString(diagnostic_field->time_role);
  }
...
TemporalMonitorPipeline::TemporalMonitorPipeline(TemporalMonitorPipelineConfig config)
    : config_(std::move(config)),
      imu_sample_buffer_(std::max<std::int64_t>(config_.imu_buffer_retention_ns, 100'000'000LL)),
      transform_age_analyzer_(config_.transform_age),
      temporal_statistics_(config_.statistics) {
  imu_timing_tracker_.SetGapThresholdMs(std::max(config_.imu_gap_threshold_ms, 1.0));
  lidar_timing_tracker_.SetGapThresholdMs(std::max(config_.lidar_gap_threshold_ms, 1.0));
...
void TemporalMonitorPipeline::ObserveLidar(const LidarPipelineInput& input) {
  ...
  if (!latest_lidar_point_time_diagnostics_.has_value()) {
    const auto inspection = point_cloud2_field_inspector_.Inspect(input.fields, config_.point_time);
  ...
  if (input.precomputed_scan_window_estimate.has_value()) {
    latest_lidar_scan_window_estimate_ = input.precomputed_scan_window_estimate;
  } else if (lidar_point_time_field_.has_value()) {
    const auto extraction = point_cloud2_time_field_extractor_.Extract(input.cloud_view, *lidar_point_time_field_);

    auto& point_time_diagnostics = latest_lidar_point_time_diagnostics_.emplace(
        latest_lidar_point_time_diagnostics_.value_or(causal_slam::model::PointTimeDiagnostics{}));
    ...
    point_time_diagnostics.extraction_unit = causal_slam::pointcloud::ToString(extraction.time_unit);
...
void TemporalMonitorPipeline::ObserveTransform(const causal_slam::transform::TransformLookupObservation& input) {
  latest_transform_age_summaries_.clear();
  latest_transform_age_summaries_.push_back(transform_age_analyzer_.Analyze(input));
}

void TemporalMonitorPipeline::ObserveTransforms(const std::vector<causal_slam::transform::TransformLookupObservation>& inputs) {
  latest_transform_age_summaries_.clear();
...
void TemporalMonitorPipeline::ObserveCloudDecision(const causal_slam::statistics::CloudDecisionEvent& event) {
...
TemporalMonitorPipelineSnapshot TemporalMonitorPipeline::BuildSnapshot(std::int64_t now_ns) {
