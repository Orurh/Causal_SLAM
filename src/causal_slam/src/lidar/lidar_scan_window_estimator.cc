...
void LidarScanWindowEstimator::SetConfig(LidarScanWindowEstimatorConfig config) {
  config_ = config;
...
LidarScanWindowEstimate LidarScanWindowEstimator::Estimate(std::int64_t header_stamp_ns) {
  ...
  const double measured_duration_ms = NanosecondsToMilliseconds(measured_duration_ns);
