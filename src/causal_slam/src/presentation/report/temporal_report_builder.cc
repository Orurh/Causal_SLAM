...
ReportSection BuildHealthSection(
  ...
  section.metrics.push_back(Metric(
      "status", causal_slam::telemetry::ToString(snapshot.overall_status)));
  return section;
}

ReportSection BuildMapUpdateSection(
    const causal_slam::diagnostics::TemporalDiagnosticSnapshot& snapshot) {
  auto section = MakeSection("map_update", "Map update");
  section.metrics.push_back(Metric(
      "allowed", BoolString(snapshot.map_update_decision.map_update_allowed)));
  section.metrics.push_back(Metric(
      "reason",
...
ReportSection BuildLidarScanWindowSection(
    const causal_slam::diagnostics::TemporalDiagnosticSnapshot& snapshot) {
  ...
  if (!snapshot.observation.lidar_scan_window.has_value()) {
    ...
    section.metrics.push_back(
        Metric("reason", "no_lidar_scan_received_yet"));
    return section;
  }
  ...
  section.metrics.push_back(Metric(
      "source", causal_slam::lidar::ToString(scan_window.source)));
  section.metrics.push_back(Metric(
      "confidence", causal_slam::lidar::ToString(scan_window.confidence)));
  section.metrics.push_back(
      Metric("duration_ms", ToReportString(scan_window.duration_ms)));
  section.metrics.push_back(Metric("reason", scan_window.reason));

  return section;
}

ReportSection BuildImuCoverageSection(
    const causal_slam::diagnostics::TemporalDiagnosticSnapshot& snapshot) {
  ...
  section.metrics.push_back(
      Metric("status", causal_slam::coverage::ToString(coverage.health)));
  section.metrics.push_back(Metric(
      "samples_in_window", ToReportString(coverage.imu_count_in_window)));
  section.metrics.push_back(
      Metric("coverage_ratio", ToReportString(coverage.coverage_ratio)));
  section.metrics.push_back(
      Metric("max_gap_inside_ms", ToReportString(coverage.max_gap_inside_ms)));

  return section;
}

ReportSection BuildStreamsSection(
    const causal_slam::diagnostics::TemporalDiagnosticSnapshot& snapshot) {
  ...
  for (const auto& stream : snapshot.observation.streams) {
    ...
    section.rows.push_back(MakeMetricRow(
        causal_slam::telemetry::ToString(stream.id),
        causal_slam::telemetry::ToString(summary.health),
        {
  ...
  for (const auto& transform_age : snapshot.observation.transform_ages) {
    section.rows.push_back(MakeMetricRow(
        ...
        causal_slam::telemetry::ToString(transform_age.health),
        {
            ...
            Metric("receive_delay_ms",
                   ToReportString(transform_age.receive_delay_ms)),
        },
        transform_age.reason,
        transform_age.adapter_detail));
  }

  return section;
}

void MaybeAppendLidarPointTimeSection(
    const causal_slam::diagnostics::TemporalDiagnosticSnapshot& snapshot,
  ...
  section.metrics.push_back(
      Metric("candidate", BoolString(point_time.has_time_candidate)));
  section.metrics.push_back(
      Metric("supported", BoolString(point_time.has_supported_time_field)));
...
ReportSection BuildIssuesSection(
    const causal_slam::diagnostics::TemporalDiagnosticSnapshot& snapshot) {
...
void RenderNumericStatsAsMetrics(
...
ReportRow BuildNumericStatsRow(
    std::string label,
    const causal_slam::statistics::NumericStats& stats) {
  ReportRow row;
  row.label = std::move(label);
...
void AppendStreamTimingStatisticsRows(
...
ReportSection BuildCloudDecisionsSection(
    const causal_slam::statistics::CloudDecisionStatistics& stats) {
  auto section = MakeSection(
      "cloud_decisions",
      "Cloud decisions",
      "no cloud decisions observed");

  section.metrics.push_back(Metric("total", ToReportString(stats.total_count)));
  section.metrics.push_back(
      Metric("forwarded", ToReportString(stats.forwarded_count)));
  section.metrics.push_back(
      Metric("blocked", ToReportString(stats.blocked_count)));
  section.metrics.push_back(
      Metric("blocked_warmup", ToReportString(stats.blocked_warmup_count)));
  section.metrics.push_back(
      Metric("blocked_by_gate", ToReportString(stats.blocked_by_gate_count)));
  ...
  for (const auto& event : stats.recent_blocked_events) {
    ...
    if (event.has_scan_window) {
      metrics.push_back(
          Metric("scan_duration_ms", ToReportString(event.scan_duration_ms)));
      metrics.push_back(Metric("scan_source", event.scan_window_source));
      metrics.push_back(
          Metric("scan_confidence", event.scan_window_confidence));
    }

    if (event.has_imu_coverage) {
      metrics.push_back(Metric(
          "imu_samples", ToReportString(event.imu_samples_in_window)));
      metrics.push_back(
          Metric("imu_coverage", ToReportString(event.imu_coverage_ratio)));
      metrics.push_back(Metric(
          "imu_max_gap_ms", ToReportString(event.imu_max_gap_inside_ms)));
    }

    auto row = MakeMetricRow(
        "blocked cloud #" + ToReportString(event.sequence_id),
        causal_slam::telemetry::ToString(event.health),
        std::move(metrics),
        event.reason,
        JoinStrings(event.fault_reasons, ", "));
    row.collapsed = true;
    section.rows.push_back(std::move(row));
  }
...
ReportSection BuildStatisticsWindowSection(
    std::string id,
    std::string title,
    const causal_slam::statistics::TemporalWindowStatistics& stats) {
  ...
  section.metrics.push_back(
      Metric("samples", ToReportString(stats.sample_count)));
  ...
  section.rows.push_back(MakeMetricRow(
      ...
      "",
      {
          Metric("point_time_field",
                 ToReportString(
                     stats.scan_window_sources.point_time_field_count)),
          Metric("measured_header_period",
                 ToReportString(
                     stats.scan_window_sources.measured_header_period_count)),
          Metric("assumed_fixed_duration",
                 ToReportString(
                     stats.scan_window_sources.assumed_fixed_duration_count)),
          Metric("driver_metadata",
                 ToReportString(
                     stats.scan_window_sources.driver_metadata_count)),
      }));
  ...
  section.rows.push_back(MakeMetricRow(
      ...
      {
          Metric("HIGH",
                 ToReportString(stats.scan_window_confidence.high_count)),
          Metric("MEDIUM",
                 ToReportString(stats.scan_window_confidence.medium_count)),
          Metric("LOW",
                 ToReportString(stats.scan_window_confidence.low_count)),
      }));

  AppendStreamTimingStatisticsRows(stats, &section);

  section.rows.push_back(
      BuildNumericStatsRow("imu_coverage_ratio", stats.imu_coverage_ratio));
  section.rows.push_back(
      BuildNumericStatsRow("imu_samples_in_window", stats.imu_samples_in_window));
  section.rows.push_back(
      BuildNumericStatsRow("imu_max_gap_inside_ms", stats.imu_max_gap_inside_ms));
...
ReportDocument TemporalReportBuilder::BuildDiagnosticsReport(
  ...
  document.sections.push_back(BuildMapUpdateSection(snapshot));
...
ReportDocument TemporalReportBuilder::BuildStatisticsReport(
  ...
  document.sections.push_back(
      BuildCloudDecisionsSection(snapshot.cloud_decisions));

  for (const auto& window : windows) {
    document.sections.push_back(BuildStatisticsWindowSection(
        std::string{window.id}, std::string{window.title}, *window.stats));
  }
