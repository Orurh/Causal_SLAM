#include "lidar_scan_window_estimator.h"
...
LidarScanWindowEstimator::LidarScanWindowEstimator(LidarScanWindowEstimatorConfig config) : config_(config) {}

void LidarScanWindowEstimator::SetConfig(const LidarScanWindowEstimatorConfig& config) {
  config_ = config;
  previous_header_stamp_ns_.reset();
}

LidarScanWindowEstimate LidarScanWindowEstimator::Estimate(std::int64_t header_stamp_ns) {
  if (!config_.prefer_measured_header_period) {
    previous_header_stamp_ns_ = header_stamp_ns;
    return BuildFallbackEstimate(header_stamp_ns, "measured_header_period_disabled");
  }
  ...
  const std::int64_t measured_duration_ns = header_stamp_ns - *previous_header_stamp_ns_;
  previous_header_stamp_ns_ = header_stamp_ns;

  if (measured_duration_ns <= 0) {
    return BuildFallbackEstimate(header_stamp_ns, "invalid_measured_header_period");
  }

  const double measured_duration_ms = causal_slam::core::NanosecondsToMilliseconds(measured_duration_ns);

  if (measured_duration_ms < config_.min_measured_scan_duration_ms || measured_duration_ms > config_.max_measured_scan_duration_ms) {
